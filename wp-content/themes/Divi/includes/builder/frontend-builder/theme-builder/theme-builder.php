...
function et_theme_builder_store_template( $theme_builder_id, $template, $allow_default ) {
	...
	if ( $use_on ) {
		$existing_use_on = get_post_meta( $post_id, $key, false );

		if ( ! is_array( $existing_use_on ) ) {
			$existing_use_on = array();
		}

		if ( $existing_use_on ) {
			foreach ( $existing_use_on as $existing_use_on_id ) {
				if ( ! in_array( $existing_use_on_id, $use_on, true ) ) {
					delete_post_meta( $post_id, '_et_use_on', $existing_use_on_id );
				}
			}
		}

		foreach ( $use_on as $condition ) {
			if ( ! in_array( $condition, $existing_use_on, true ) ) {
	...
	if ( $exclude_from ) {
		$existing_exclude_from = get_post_meta( $post_id, $key, false );

		if ( ! is_array( $existing_exclude_from ) ) {
			$existing_exclude_from = array();
		}

		if ( $existing_exclude_from ) {
			foreach ( $existing_exclude_from as $existing_exclude_from_id ) {
				if ( ! in_array( $existing_exclude_from_id, $exclude_from, true ) ) {
					delete_post_meta( $post_id, '_et_exclude_from', $existing_exclude_from_id );
				}
			}
		}

		foreach ( $exclude_from as $condition ) {
			if ( ! in_array( $condition, $existing_exclude_from, true ) ) {
