...
function et_theme_builder_api_save() {
	...
	if ( $last_request ) {
		...
		$existing_templates = get_post_meta( $theme_builder_id, '_et_template' );

		if ( is_array( $existing_templates ) ) {
			$existing_templates = array_map( 'intval', $existing_templates );
		}
		...
		foreach ( $cached_templates as $template ) {
			...
			// Add template ID into $updated_templates for later use to delete existing template.
			$updated_templates[] = $new_post_id;

			// Add mapping template ID to theme builder ID.
			if ( ! in_array( $new_post_id, $existing_templates, true ) ) {
				add_post_meta( $theme_builder_id, '_et_template', $new_post_id );
			}
		}

		// Delete existing templates.
		if ( $existing_templates ) {
			foreach ( $existing_templates as $existing_template_id ) {
				if ( ! in_array( $existing_template_id, $updated_templates, true ) ) {
					// Delete existing template when the ID is not exist in cached templates.
					// Deletion will be done individually to prevent templates lost
					// when user Stopping/Interrupting/Refreshing the saving process.
					// @see https://github.com/elegantthemes/Divi/issues/23393.
					delete_post_meta( $theme_builder_id, '_et_template', $existing_template_id );
				}
			}
		}
